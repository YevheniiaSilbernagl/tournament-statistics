...
class Application @Inject()(ws: WSClient) extends Controller {
  ...
  def generateImages(player1: (String, Option[String]), player2: (String, Option[String])): (String, String) = {
    ...
    def generateImage(player: (String, Option[String]), side: String): String = {
      ...
      if (targetFile.exists()) targetFile.getName else {
          ...
          case bg if bg.exists() =>
            currentPlayers.filter(p => p._1 == player._1).flatMap(_._2).map(getDeck).headOption match {
              case Some(deck) =>
                ...
                def scale(image: BufferedImage, scale: Double): BufferedImage = {
                  val w = image.getWidth * scale
                  val h = image.getHeight * scale
                  val dest = new BufferedImage(w.intValue(), h.intValue(), BufferedImage.TYPE_INT_RGB)
                ...
                def drawCard(name: String, quantity: Int) = {
                  val cardFile = Option(new File(s"$sourceDir/images/cards/$name")).filter(_.exists())
                    .getOrElse(new File(s"$sourceDir/images/MISSING.png"))
                  val cardImage = scale(ImageIO.read(cardFile), cardScale)
                  ...
                  val quantityImage = scale(qImage, quantityScale)
